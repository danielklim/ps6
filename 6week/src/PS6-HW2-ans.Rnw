...
\begin{multicols}{2}
<<echo=F>>=
print('t-test 1960, delta=0')
ex3(1960)
@
\columnbreak

<<echo=F>>=
print('t-test 2000, delta=0')
ex3(2000)
@
\end{multicols}

Students only need to show p-value. A correct interpretation is that the p-vals
less than 0.05 allow us to reject the null hypothesis that the samples are drawn
from populations with the exact same mean. They must also say that this is NOT
interesting because even small differences in population means could lead to
statistically significant results on this test ('interesting' is an OK
substitute for 'statistically significant').


\section*{Exercise 4: 20 points}
1960: $\Delta \approx 24.5$ is the maximum distance between the population means
of South and North that is statistically significant.
2000: $\Delta \approx 3.25$

See below plots for reasining.
...
stated, and whether word limits (250-750) are met.
