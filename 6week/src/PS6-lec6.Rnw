...
\vspace{0.2in}
Based on this plot, look for countries with `high==1' and GDP/cap $<
\$15000$ in 1990
...
\begin{itemize}
  \item We've got our culprits!
  \item Let's use a line graph to see exactly what happened.
  \end{itemize}
...
First, look only at the first row.
\footnotesize
<<csbro1, eval=F>>=
Xs = 1966:1997
Ys = declined[1, 2:33]
...
\begin{description}
  ...
  numbers by looking at the data)
...
@
  ...
  There was a decline in living standards from 1981-1986, then a
...
%\begin{frame}[fragile]
%You can do this exercise for each country in `declined' by simply changing the
%row number.
%\vspace{0.2in}
%
%Not knowing anything about \Sexpr{cName}, you could read up on its history to
%see why we observe these patterns in the data.
%\vspace{0.2in}
%
%We can also plot all the declined countries on a single plot\ldots we'll get to
%that next lecture.
%\end{frame}
%%%%%%%%%%%%%%
%
...
%  \column{0.3\textwidth}
%  
%  We left off looking at a line graph for just Bahrain.
%  \vspace{0.2in}
%  
%  There were 4 other countries, and we wanted to figure out how to plot them all
%  on a single plot, rather than one by one.
%
%  \column{0.7\textwidth}
%  
%  \setkeys{Gin}{width=\textwidth}
...
\begin{itemize}
  \item can take any value
  \item average amount that $X$ \& $Y$ deviate together
  \item magnitude is not worth interpreting
  \begin{itemize}
    \item changes with unit of measurement for either $X$ or $Y$
    \item is on scale of $X \times Y$
    \end{itemize}
  \item mainly used in formulas
  \item $Cov(X, X) = Var(X)$
...
\begin{itemize}
  ...
  \item how we typically \textit{talk} about bivariate relationships
...
Note:
\begin{itemize}
  \item we only have '75-'96 data for Bahrain. In
  wide-form, there were NAs for values outside that range. In long-form, there
  are no rows for years where we don't have data.
  \item see example 1 for a recap of the array/matrix notation
  \end{itemize}
...
\begin{itemize}
  \item whether '81-'86 is special is a \textit{qualitative} judgement. All this
...
%
%\begin{frame}[fragile]
%Our approach (edit this slide later)
%\begin{itemize}
%  \item what it does
%  \item how it works
%  \item intricacies and caveats
%  \end{itemize}
%\vspace{0.2in}
%
%For today, just a simple example to whet the appetite -- a full intro will
%come next lecture.
%\end{frame}
%%%%%%%%%%%%%%
%
%\begin{frame}[fragile]
%Let's model the linear relationship between year and GDP/cap for Bahrain. Line
%1 runs the model and the next line gives us our model estimates.
%\footnotesize
%<<>>=
...
%  
%  \column{0.35\textwidth}
%  The regression table from the previous slide let's us model
%  GDP/cap like so.\vspace{0.2in}
%  
%  We will discuss what all of this means next time.
%  
%\end{columns}
%\end{frame}
%%%%%%%%%%%%%
