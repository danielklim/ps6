...
\begin{itemize}
  ...
  \item p-value on $\beta_0$ is \Sexpr{round(pvals[1], 4)} -- \texit{not}
...
\begin{itemize}
  ...
  effect of height on weight is \texit{not zero} (i.e.
  \Sexpr{100-round(pvals[2], 3)*100}\% sure the null hypothesis can be
  rejected)\vspace{0.1in}
  \item Since we created this fake data, we \texit{know} that the true effect of
