...
\begin{frame}[fragile]
\frametitle{Density}
\textbf{Density is only meaningful for an infinitesimally small slice of values}
\vspace{0.1in}
...
\begin{itemize}
  ...
  expect since we specified the mean to be 0.\vspace{0.1in}
...
\begin{description}
  ...
  scalar or a vector. Here, we passed it a scalar: 1\vspace{0.1in}
  \item[arg 2:] the mean of the data (again, 0)\vspace{0.1in}
  \item[arg 3:] the SD of the data (again, 1)\vspace{0.1in}
  \end{description}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%
...
Given the name of the function, we might think that if we drew 1000 random draws
from the same distribution, about \Sexpr{round(densAt1, 2)*1000} of them should equal 1\ldots
right?
...
\textbf{Reason 1:} The normal distribution can take on \text{any} value,
...
\textbf{Reason 2:} Even though we can think of it as a measure of
likelihood, \textbf{density IS NOT a probability}.
\vspace{0.2in}

\begin{itemize}
  \item We want probability (it's an intuitive measure) -- how do we get there?
  \item What use is the density?
\end{itemize}
...
probability that the data takes on a value less than or equal to
b.}\vspace{0.1in}
  \item The `cumulative' in the name comes from the fact that it is the accumulation of
the density function from $-\infty$ to $b$.\vspace{0.1in}
\item This we can easily demonstrate using our fake data.
  \end{itemize}
...
\begin{frame}[fragile]
How many of our simulated observations are less than 1?\vspace{0.1in}
\footnotesize
<<>>=
...
\begin{frame}[fragile]
We use `pnorm' to get the CDF from -$\infty$ to $b$ (arg 1)\vspace{0.1in}
\footnotesize
<<>>=
ThLt1 = pnorm(1, 0, 1)
ThLt1 * 1000
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
Using the simulated data:\vspace{0.1in}
\footnotesize
<<>>=
a = -1
b = 1
...
\begin{frame}[fragile]
\frametitle{Density plots}
To create:\vspace{0.1in}
\begin{itemize}
  \item determine the domain of X you're interested in.\vspace{0.1in}
  \item plug the range of values you're interested in into $f_X$.\vspace{0.1in}
...
\vspace{0.2in}
\begin{itemize}
  \item The X-axis will be the same.\vspace{0.1in}
  \item The Y-axis will range from 0 to 1 for the CDF. This is because of the
  definition of the CDF \vspace{0.1in}
  \begin{itemize}
    \item CDF is the probability/proportion of observations that will
  be below some value of x. \vspace{0.1in}
  \item Probability cannot be less than 0 or greater than 1,
...
\begin{itemize}
  \item requires 2 arguments -- 1 for an x coordinate, another for
  y.\vspace{0.1in}
...
\begin{description}
    \item[scalar]: `plot(2, 3)' to plot a single point with x=2,
    y=3\vspace{0.1in}
...
\frametitle{The plot function}
  ...
  \begin{itemize}
  \item call `plot' with ``type='l','' to get lines, rather than points.
  \vspace{0.1in}
    \begin{itemize}
      \item Try `plot(c(1, 2), c(3, 4), type='l')'
      \end{itemize}\vspace{0.1in}
  \item `hist' \& `boxplot' are specialized versions of `plot'. \vspace{0.1in}
      \begin{itemize}
      \item Any optional argument they can take, `plot' can -- e.g. `col',
      `lwd', `lty', `xlab', `ylab', `main', etc.
      \end{itemize}
   \end{itemize}
...
text(2, 0.15, 'Area under curve = 1')
@
...
\section{Normal distribution parameters}
...
\begin{itemize}
  \item The \_norm functions each took another argument\ldots\vspace{0.1in}
...
them as values to pass to a programming function\vspace{0.1in}
...
\begin{frame}[fragile]
To recap: our goals for this theory/math heavy lecture\vspace{0.1in}

\begin{itemize}
  \item Gain a rudimentary understanding of (one of) the most important
  distributions in statistics\vspace{0.1in}
  \item Lay groundwork for next lecture where\ldots\vspace{0.1in}
  \item we will acquire the tools necessary to rigourously test the assertion
  that the turnout data are normal
  \end{itemize}
